...
enum class SettingsUiEvent { RATES_SAVED, PASSWORD_SAVED, PASSWORD_INVALID, HISTORY_DELETED, FAILED }
...
class SettingsViewModel(
    ...
    private fun perform(success: SettingsUiEvent, action: suspend () -> Unit) {
        ...
        viewModelScope.launch {
            try {
                ...
                _events.send(SettingsUiEvent.FAILED)
            } finally {
    ...
    fun saveRates() {
        ...
        if (!dollarValid || !euroValid) return

        perform(SettingsUiEvent.RATES_SAVED) {
            dataStorage.save(
    ...
    fun savePassword() {
        val state = _uiState.value
        if (!validator.validateEmpty(state.password).isSuccess || state.password != state.passwordConfirmation) {
            _events.trySend(SettingsUiEvent.PASSWORD_INVALID)
            return
        }
        perform(SettingsUiEvent.PASSWORD_SAVED) {
    ...
    fun deleteIncomeHistory() {
        perform(SettingsUiEvent.HISTORY_DELETED) { historyRepository.deleteAll() }
    }
